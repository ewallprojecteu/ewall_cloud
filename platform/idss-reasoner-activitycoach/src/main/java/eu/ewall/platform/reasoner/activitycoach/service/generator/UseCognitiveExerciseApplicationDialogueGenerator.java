...
public class UseCognitiveExerciseApplicationDialogueGenerator extends DialogueGenerator {
	
	private static final String LOGTAG = "UseCognitiveExerciseApplicationDialogueGenerator";
	private Logger logger;
	private List<String> otherReminderDialogueTypes;
	
	public UseCognitiveExerciseApplicationDialogueGenerator(VCService vcService) {
		super(vcService);
	...
	public VCMessageAvailable generateDialogueAvailable(IDSSUserProfile idssUserProfile) {
		...
		try {
			postponeTime = vcDataCollection.getPostponeCognitiveExerciseApplicationReminder();
			...
			logger.error("["+username+"] Failed to read Cognitive Exercise Application reminder postpone information from InteractionModel.",e);
			return null;
		}
		...
		try {
			lastInteraction = vcDataCollection.getLastInteractionCognitiveExerciseApplication();
		} catch (Exception e) {
			logger.error("["+username+"] Failed to read last interaction with Cognitive Exercise Application information from InteractionModel.",e);
			return null;
		}
	...
	protected String getDialogueTypeId() {
		return DialogueConstants.DT_USE_COGNITIVE_EXERCISE_APPLICATION;
	}
