...
public class UseHealthApplicationDialogueGenerator extends DialogueGenerator {
	
	private static final String LOGTAG = "UseHealthApplicationDialogueGenerator";
	private Logger logger;
	private List<String> otherReminderDialogueTypes;
	
	public UseHealthApplicationDialogueGenerator(VCService vcService) {
		super(vcService);
	...
	public VCMessageAvailable generateDialogueAvailable(IDSSUserProfile idssUserProfile) {
		...
		try {
			postponeTime = vcDataCollection.getPostponeHealthApplicationReminder();
			...
			logger.error("["+username+"] Failed to read My Health application reminder postpone information from InteractionModel.",e);
			return null;
		}
		...
		try {
			lastInteraction = vcDataCollection.getLastInteractionMyHealthApplication();
		} catch (Exception e) {
			logger.error("["+username+"] Failed to read last interaction with My Health application information from InteractionModel.",e);
			return null;
		}
	...
	protected String getDialogueTypeId() {
		return DialogueConstants.DT_USE_HEALTH_APPLICATION;
	}
