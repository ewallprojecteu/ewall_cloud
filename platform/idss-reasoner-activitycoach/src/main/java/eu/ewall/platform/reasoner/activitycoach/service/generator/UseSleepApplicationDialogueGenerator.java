...
public class UseSleepApplicationDialogueGenerator extends DialogueGenerator {
	
	private static final String LOGTAG = "UseSleepApplicationDialogueGenerator";
	private Logger logger;
	private List<String> otherReminderDialogueTypes;
	
	public UseSleepApplicationDialogueGenerator(VCService vcService) {
		super(vcService);
		...
		otherReminderDialogueTypes.add(DialogueConstants.DT_USE_DOMOTICS_APPLICATION);
	...
	public VCMessageAvailable generateDialogueAvailable(IDSSUserProfile idssUserProfile) {
		...
		if(queue.hasDialogueAvailable(getDialogueTypeId())) {
			if(queue.isExpiredDialogue(getDialogueTypeId(),now)) {
				...
				logger.info("["+username+"] Dialogue of type '"+getDialogueTypeId()+"' in queue active, returning available message from queue.");
		...
		for(String otherReminderType : otherReminderDialogueTypes) {
			if(queue.hasDialogueAvailable(otherReminderType)) {
				if(queue.isExpiredDialogue(otherReminderType,now)) {
					...
					logger.info("["+username+"] Another application reminder is already in the queue, skipping Sleep Application reminder.");
					return null;
				}
			}
		}
		...
		if(dialogueManager.hasActiveDialogue()) {
			...
			if(activeDialogueTypeId.equals(getDialogueTypeId())) {
				// Dialogue is active, so should not be generated
				return null;
			}
		}
		...
		try {
			postponeTime = vcDataCollection.getPostponeSleepApplicationReminder();
			...
			logger.error("["+username+"] Failed to read sleep application reminder postpone information from InteractionModel.",e);
			return null;
		}
		...
		try {
			lastInteraction = vcDataCollection.getLastInteractionSleepApplication();
		} catch (Exception e) {
			logger.error("["+username+"] Failed to read last interaction with sleep application information from InteractionModel.",e);
			return null;
		}
	...
	protected String getDialogueTypeId() {
		return DialogueConstants.DT_USE_SLEEP_APPLICATION;
	}
