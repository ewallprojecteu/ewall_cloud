...
public class UseCalendarApplicationDialogueGenerator extends DialogueGenerator {

	private static final String LOGTAG = "UseCalendarApplicationDialogueGenerator";
	private Logger logger;
	private List<String> otherReminderDialogueTypes;
	
	public UseCalendarApplicationDialogueGenerator(VCService vcService) {
		super(vcService);
	...
	public VCMessageAvailable generateDialogueAvailable(IDSSUserProfile idssUserProfile) {
		...
		try {
			postponeTime = vcDataCollection.getPostponeCalendarApplicationReminder();
			...
			logger.error("["+username+"] Failed to read Calendar Application reminder postpone information from InteractionModel.",e);
			return null;
		}
		...
		try {
			lastInteraction = vcDataCollection.getLastInteractionCalendarApplication();
		} catch (Exception e) {
			logger.error("["+username+"] Failed to read last interaction with Calendar Application information from InteractionModel.",e);
			return null;
		}
	...
	protected String getDialogueTypeId() {
		return DialogueConstants.DT_USE_CALENDAR_APPLICATION;
	}
