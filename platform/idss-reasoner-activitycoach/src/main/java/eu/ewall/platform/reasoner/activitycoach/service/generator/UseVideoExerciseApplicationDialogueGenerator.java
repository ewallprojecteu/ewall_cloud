...
public class UseVideoExerciseApplicationDialogueGenerator extends DialogueGenerator {

	private static final String LOGTAG = "UseVideoExerciseApplicationDialogueGenerator";
	private Logger logger;
	private List<String> otherReminderDialogueTypes;
	
	public UseVideoExerciseApplicationDialogueGenerator(VCService vcService) {
		super(vcService);
	...
	public VCMessageAvailable generateDialogueAvailable(IDSSUserProfile idssUserProfile) {
		...
		try {
			postponeTime = vcDataCollection.getPostponeVideoExerciseApplicationReminder();
			...
			logger.error("["+username+"] Failed to read Video Exercise Application reminder postpone information from InteractionModel.",e);
			return null;
		}
		...
		try {
			lastInteraction = vcDataCollection.getLastInteractionVideoExerciseApplication();
		} catch (Exception e) {
			logger.error("["+username+"] Failed to read last interaction with Video Exercise Application information from InteractionModel.",e);
			return null;
		}
	...
	protected String getDialogueTypeId() {
		return DialogueConstants.DT_USE_VIDEO_EXERCISE_APPLICATION;
	}
