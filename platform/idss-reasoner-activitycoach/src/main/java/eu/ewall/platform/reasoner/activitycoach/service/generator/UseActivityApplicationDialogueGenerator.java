...
public class UseActivityApplicationDialogueGenerator extends DialogueGenerator {
	
	private static final String LOGTAG = "UseActivityApplicationDialogueGenerator";
	private Logger logger;
	private List<String> otherReminderDialogueTypes;
	
	public UseActivityApplicationDialogueGenerator(VCService vcService) {
		super(vcService);
	...
	public VCMessageAvailable generateDialogueAvailable(IDSSUserProfile idssUserProfile) {
		...
		try {
			postponeTime = vcDataCollection.getPostponeActivityApplicationReminder();
			...
			logger.error("["+username+"] Failed to read Activity Application reminder postpone information from InteractionModel.",e);
			return null;
		}
		...
		try {
			lastInteraction = vcDataCollection.getLastInteractionMyActivityApplication();
		} catch (Exception e) {
			logger.error("["+username+"] Failed to read last interaction with Activity Application information from InteractionModel.",e);
			return null;
		}
	...
	protected String getDialogueTypeId() {
		return DialogueConstants.DT_USE_ACTIVITY_APPLICATION;
	}
