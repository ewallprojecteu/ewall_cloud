...
import eu.ewall.platform.middleware.cloudgateway.services.SpeakerSensingServiceImpl;
 ...
 */
@RestController
@RequestMapping("/sensingenvironments/{sensing_environment_id}/devices/{device_id}/speaker")
public class SpeakerSensingController {
	...
	public ResponseEntity<String> addNewContent(@RequestBody String content,
		...
		try {
			...
			SpeakerSensing speakerSensing = (SpeakerSensing) DM2JsonObjectMapper
					.readValueAsString(content, SpeakerSensing.class);

			speakerSensingService.addSpeakerSensing(sensing_environment_id,
					speakerSensing);
	...
	public ResponseEntity<Long> getLastCollectionsTimestamp(
		...
		try {
			long lastTimestamp = speakerSensingService
