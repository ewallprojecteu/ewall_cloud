...
public class ToStringMethodsTest {
	...
	public static void main(String[] args) {
		...
		TimeInterval testTimeInterval = new TimeInterval(new Date(2014, 8, 26,
				10, 45).getTime(), new Date(2014, 8, 27, 11, 12).getTime());
		gamingActivity.setTimeInterval(testTimeInterval);

		activitiesSubProfile.addActivityToActivitiesSubProfile(gamingActivity);
		activitiesSubProfile.addActivityToActivitiesSubProfile(new SkypeCall(
				testTimeInterval));
		activitiesSubProfile
				.addActivityToActivitiesSubProfile(new MediaConsumingActivity(
		...
		testHealthMedications
				.addMedicationToMedicatiosList("some medication name");

		HealthSubProfile healthSubProfile = new HealthSubProfile(
				testHealthMeasurements, new HealthImpairments(false, true,
						false, false, false), testHealthMedications, null);
		userProfile.setHealthSubProfile(healthSubProfile);

		AudioPreferences testAudioPreferences = new AudioPreferences(
				Intensity.MEDIUM, VCardGenderType.FEMALE, Intensity.LOW);
		userProfile
				.setUserPreferencesSubProfile(new UserPreferencesSubProfile(
						testAudioPreferences, null, null));

		testUser.setUserProfile(userProfile);
		testUser.setUserRole(UserRole.PRIMARY_USER);

		System.out
				.println("User with all subprofiles including measurements and locations:\n"
						+ testUser.toString());
