...
// Modules
    ...
    function touchSelectorDirective(touchSelectConfig, elementFromPoint) {
        return {
            restrict: 'A',
            replace: true,
            template: '<div ng-class="selectorClasses" ng-init="initMask(value)" ng-mousedown="showOptions($event)"'+
            ...
            controller: ['$scope', '$element', '$attrs', '$document', '$sce', 'radialPosition',
                function($scope, $element, $attrs, $document, $sce, radialPosition) {
                    ...
                    $scope.showOptions = function($event) {
                    	//console.log('touchStart/click event: %O', $event);
                    	var classList = $event.currentTarget.className; 

                         var index = classList.indexOf('hint');
                         if(index != -1){
                             return;
                         }
                    ...
                    $scope.$on('selectedOption', function(event, index) {
                    	//console.log('touchEnd event %O', event);
                    ...
                    $scope.touchSelectOption = function(event) {
                    	//console.log('touchSelectOption: %O', event);
                        event.preventDefault();
